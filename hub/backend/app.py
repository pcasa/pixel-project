...
# Gemini Live disabled: Project Pixel uses Claude API as the brain instead.
USE_GEMINI_LIVE = False

# Default Claude model for voice turns. Override via bot_settings.json "model" key.
...
def get_bot_settings(device_id: str):
    ...
    return {
        ...
        # Claude model override — defaults to DEFAULT_CLAUDE_MODEL if not set or not a claude model
        "model": bot_conf.get("model", DEFAULT_CLAUDE_MODEL),
    }
...
if not get_anthropic_api_key():
    print(
        "WARNING: No Anthropic API key — set ANTHROPIC_API_KEY in .env or configure in Hub settings."
    )
if not get_gemini_api_key():
    print(
        "WARNING: No Gemini API key (used for STT + TTS) — set GEMINI_API_KEY in .env or configure in Hub settings."
    )
...
async def stream_chat_turn_response(
    ...
    *,
    ...
    """One Claude turn via text. Used by dashboard text commands and presence greetings.

    Returns (full_assistant_text, stream_id).
    """
    global _main_async_loop
    _main_async_loop = asyncio.get_running_loop()
    ...
    async with lock:
        ...
        model = bot_settings.get("model", DEFAULT_CLAUDE_MODEL)
        ...
        # message_content may be a string or list; extract text for Claude
        if isinstance(message_content, str):
            user_text = message_content
        elif isinstance(message_content, list):
            # Flatten list parts to text (audio/video parts from wake path are pre-transcribed)
            parts_text = " ".join(
                str(p) for p in message_content if isinstance(p, str)
            )
            user_text = parts_text.strip() or str(message_content)
        else:
            user_text = str(message_content)

        async def _first_token_cb() -> None:
        ...
        try:
            session = claude_session.get_session(device_id)
            full_text = await session.send_message(
                user_text,
                system_prompt=system_instruction,
                model=model,
                api_key=anthropic_key,
                tool_declarations=_build_claude_tool_declarations(),
                on_tool_call=lambda did, name, inp: _execute_claude_tool(did, stream_id, name, inp),
                on_first_token=_first_token_cb,
                max_tool_rounds=max_tool_rounds,
            )
            # Stream the full response as a single delta for dashboard compatibility
            if full_text:
                await _first_token_cb()
                await manager.broadcast({
    ...
    end_msg = {
        ...
        "device_id": device_id,
        "data": full_text,
        "device_stream_connected": device_stream_is_live(device_id),
    }
...
async def _process_presence_jpeg(device_id: str, jpeg_bytes: bytes) -> None:
    ...
    try:
        full_text, _stream_id = await stream_chat_turn_response(device_id, msg)
        esp32_ws = get_active_esp32_socket(device_id)
        if esp32_ws:
            try:
                await esp32_ws.send_text(json.dumps({"status": "success", "reply": full_text}))
            except Exception as e:
...
async def _execute_claude_tool(device_id: str, stream_id: str, tool_name: str, tool_input: dict) -> Any:
    """Execute a tool call from Claude and return the result dict."""
    turn_state = turn_tool_state_by_device.setdefault(device_id, {})

    if tool_name == "face_animation":
        if not turn_state.get("face_animation"):
            turn_state["face_animation"] = True
            anim = str(tool_input.get("animation", ""))
            _schedule_face_animation_to_esp32(device_id, anim)
        return {"ok": True}

    if tool_name == "soul_replace":
        return persona.replace_soul_markdown(device_id, tool_input.get("markdown", ""))

    if tool_name == "memory_replace":
        return persona.replace_memory_markdown(device_id, tool_input.get("new_memory", ""))

    if tool_name == "persona_replace":
        return persona.replace_persona_target_markdown(
            device_id, tool_input.get("file", ""), tool_input.get("markdown", "")
        )

    if tool_name == "daily_log_append":
        return persona.append_daily_log_line(device_id, tool_input.get("line", ""))

    if tool_name == "bootstrap_complete":
        return persona.delete_bootstrap_file(device_id)

    return {"error": f"Unknown tool: {tool_name}"}


def _build_claude_tool_declarations() -> list[dict]:
    """All tool declarations in OmniBot OpenAPI format — Claude session converts to Anthropic format."""
    return [
        FACE_ANIMATION_FUNCTION_DECLARATION,
        persona.SOUL_REPLACE_DECLARATION,
        heartbeat_service.MEMORY_REPLACE_DECLARATION,
        persona.PERSONA_REPLACE_DECLARATION,
        persona.DAILY_LOG_APPEND_DECLARATION,
        persona.BOOTSTRAP_COMPLETE_DECLARATION,
    ]


async def _run_claude_audio_turn(
    websocket: WebSocket,
    device_id: str,
    raw_pcm: bytes,
    video_frames: list,
) -> str:
    """STT via Gemini → Claude brain → Gemini TTS → broadcast PCM + send text reply to ESP32."""
    wav_bytes = create_wav_header(len(raw_pcm), sample_rate=16000) + raw_pcm

    # Broadcast captured audio to dashboard
    await manager.broadcast({
        "type": "audio_captured",
        "data": "data:audio/wav;base64," + base64.b64encode(wav_bytes).decode("utf-8"),
    })

    gc = get_genai_client()
    if gc is None:
        raise RuntimeError("Gemini API key not configured (needed for STT).")

    anthropic_key = get_anthropic_api_key()
    if not anthropic_key:
        raise RuntimeError("Anthropic API key not configured. Set ANTHROPIC_API_KEY in .env.")

    # STT: transcribe audio with Gemini Flash
    print("Transcribing audio...", end=" ", flush=True)
    user_text = await gemini_tts_stream.transcribe_audio(gc, wav_bytes)
    if not user_text:
    ...
    # Notify ESP32 we're processing (triggers thinking animation)
    await _notify_esp32_live_first_token(device_id)

    # Claude brain: text → text
    system_prompt = resolve_effective_system_instruction(device_id)
    bot_settings = get_bot_settings(device_id)
    model = bot_settings.get("model", DEFAULT_CLAUDE_MODEL)
    if not model.startswith("claude"):
        model = DEFAULT_CLAUDE_MODEL
    ...
    print(f"Sending to Claude ({model})...", end=" ", flush=True)
    assistant_text = await session.send_message(
        user_text,
    ...
    print(f"\n>>> CLAUDE SAYS: {assistant_text}")

    # Broadcast assistant transcription to dashboard
    await manager.broadcast({
        "type": "live_transcription",
        "device_id": device_id,
        "role": "model",
    ...
    # Send text reply to ESP32 (displays on round screen)
    await websocket.send_text(json.dumps({"status": "success", "reply": assistant_text}))

    # TTS: convert assistant text to PCM and broadcast to dashboard audio output
    if assistant_text:
        print("Generating TTS...", end=" ", flush=True)
        await _notify_esp32_assistant_speech_face(device_id, {"event": "start"})
        try:
            pcm_bytes = await gemini_tts_stream.text_to_pcm(gc, assistant_text)
            if pcm_bytes:
                print(f"{len(pcm_bytes)} PCM bytes")
                await manager.broadcast({
                    "type": "live_audio_chunk",
                    "device_id": device_id,
                    "stream_id": stream_id,
                    "sample_rate": gemini_tts_stream.OUTPUT_SAMPLE_RATE,
                    "channels": 1,
                    "encoding": "pcm_s16le",
            ...
            await _notify_esp32_assistant_speech_face(device_id, {"event": "end"})
...
# Legacy name kept so remaining callers that reference it still compile.
_run_gemini_audio_turn = _run_claude_audio_turn
